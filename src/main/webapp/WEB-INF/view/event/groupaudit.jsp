...
$(function() {
	...
	$('#dg').datagrid({
		pageNumber : 1,
		pagination : true,
		url : "${pageContext.request.contextPath}/attempted/event/showNearMiss?range=公司审核",
		rownumbers : true,
		fitColumns : true,
		fit : true,
		pageSize : 10,
		...
		toolbar : [{
				...
				handler : function() {
					var str="${sessionScope['permissions']}";
					if(str.indexOf("160401")==-1){
						$("#audittool").css('display','none');
					}else{
						var row = $('#dg').datagrid('getSelected');
						if (row) {
							if(row.nearMissState=='公司审核通过'||row.nearMissState=='公司审核未通过'){
								$.messager.alert('提示','公司已经审核');
							}
							else{
								$('#win').window('open');
	...
	$('#passbtn').bind('click', function(){    
		$('#form').form('submit', {
			url:"${pageContext.request.contextPath}/attempted/event/changeStateNearMiss?range=公司审核通过&nearMissSn="+nearMissSn,
	...
	$('#impassbtn').bind('click', function(){
		$('#form').form('submit', {
			url:"${pageContext.request.contextPath}/attempted/event/changeStateNearMiss?range=公司审核未通过&nearMissSn="+nearMissSn,
			success: function(message){
				$.messager.alert('提示',message); 
				$('#dg').datagrid('reload');
			}
