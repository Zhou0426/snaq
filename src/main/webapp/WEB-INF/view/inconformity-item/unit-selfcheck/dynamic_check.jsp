...
<script type="text/javascript">
	...
	$(function(){
		...
		$('#dg').datagrid({
			...
			//title:"动态检查",
			queryParams:{checkType:'动态检查',checkerFrom:'单位'},//请求远程数据发送额外的参数
				...
				handler:function(){
					...
					if(resources.indexOf('01030101')==-1){
						...
						$('#win').window({
			 				...
			 				title:'添加不符合项-隐患',
			 				cache:false,
			 				content:'<iframe src="${pageContext.request.contextPath}/unit/selfcheck/dynamic_check_add" frameborder="0" width="100%" height="100%"/>'
			 			});
					}
				}
			},{
						...
						if(rows.length==1){
							if(loginId==rows[0].editor.personId || (rows[0].checkers.num>0 && $.inArray('${sessionScope.pId}', rows[0].checkers.personIds.split(','))!=-1) || roles.indexOf('dwxtgly')!=-1){
								$('#win').window({
									...
									title:"修改记录",
									cache:false,
									content:'<iframe src="${pageContext.request.contextPath}/unit/selfcheck/dynamic_check_update" frameborder="0" width="100%" height="100%"/>'
					...
					if(resources.indexOf('01030105')==-1){
						$('#unsafeact-add').css('display','none');
					 	...
					 	$('#win2').window({
					 		...
					 		title:'不符合项-不安全行为',
					 		cache:false,
					 		content:'<iframe src="${pageContext.request.contextPath}/unit/selfcheck/dynamic_check_add_unsafeAct" frameborder="0" width="100%" height="100%"/>'
					 	});
					}
				}
			},{
