...
<script type="text/javascript">
	function inconformityItem(index){
		...
		$('#win').window({
			...
			title:'不符合项',
			cache:false,
			content:'<iframe src="${pageContext.request.contextPath}/external/check/special_check_inconformityItem"+ frameborder="0" width="100%" height="100%"/>'
		});
	}
	$(function(){
		var resources="${sessionScope['permissions']}";
		var roles="${sessionScope['roles']}";
		var loginId="${person.personId}";
		$('#dg').datagrid({
			...
			title:"专项检查",
			queryParams:{checkerFrom:'外部'},//请求远程数据发送额外的参数
			...
			onSelect:function(index,row){
				if(resources.indexOf('01070301')==-1){
					$('#add').css('display','none');
				}
				if(resources.indexOf('01070302')==-1){
				...
				if(resources.indexOf('01070303')==-1){
				...
				handler:function(){
					...
					if(resources.indexOf('01070301')==-1){
						...
						$('#win').window({
			 				...
			 				title:'不符合项（隐患、不安全行为）',
			 				cache:false,
			 				content:'<iframe src="${pageContext.request.contextPath}/external/check/special_check_add" frameborder="0" width="100%" height="100%"/>'
			 			});
					}
				}
			},{
						...
						if(rows.length==1){
							...
							if(rows[0].editor.personId==loginId || roles.indexOf('jtxtgly')!=-1){
								$('#win').window({
									...
									title:"修改记录",
									cache:false,
									content:'<iframe src="${pageContext.request.contextPath}/external/check/special_check_update" frameborder="0" width="100%" height="100%"/>'
								});
							}else{
