...
<script type="text/javascript">
	function inconformityItem(index){
		...
		$('#win').window({
			...
			title:'不符合项',
			cache:false,
			content:'<iframe src="${pageContext.request.contextPath}/external/check/periodical_check_inconformityItem"+ frameborder="0" width="100%" height="100%"/>'
		});
	}
	$(function(){
		...
		$('#dg').datagrid({
			...
			onSelect:function(index,row){
				if(resources.indexOf('01070201')==-1){
					$('#add').css('display','none');
				}
				if(resources.indexOf('01070202')==-1){
				...
				if(resources.indexOf('01070203')==-1){
				...
				handler:function(){
					...
					if(resources.indexOf('01070201')==-1){
						...
						$('#win').window({
			 				...
			 				title:'定期检查',
			 				cache:false,
			 				content:'<iframe src="${pageContext.request.contextPath}/external/check/periodical_check_add" frameborder="0" width="100%" height="100%"/>'
			 			});
					}
				}
			},{
						...
						if(rows.length==1){
							...
							if(rows[0].editor.personId==loginId || roles.indexOf('jtxtgly')!=-1){
								$('#win').window({
									...
									title:"修改记录",
									cache:false,
									content:'<iframe src="${pageContext.request.contextPath}/external/check/periodical_check_update" frameborder="0" width="100%" height="100%"/>'								
								});
							}else{
