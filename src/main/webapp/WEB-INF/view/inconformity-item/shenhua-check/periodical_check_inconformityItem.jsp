		...
		$("#d-i").datagrid({
			...
			onSelect:function(index,row){
				if(loginId==editor || $.inArray(loginId, checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
					if(resources.indexOf('0106020501')==-1){
						$('#unsafecondition-add').css('display','none');
					}
					if(resources.indexOf('0106020502')==-1){
					...
					if(resources.indexOf('0106020503')==-1){
					...
					if(resources.indexOf('0106020501')==-1){
						...
						if(loginId==editor || $.inArray(loginId,checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
							parent.$('#child-win').window({
				 				...
				 				title:'添加不符合项-隐患',
				 				cache:false,
				 				content:'<iframe src="${pageContext.request.contextPath}/shenhua/check/periodical_check_inconformity_add" frameborder="0" width="100%" height="100%"/>'
				 			});
						}else{
						...
						if(rows2.length==1){
							...
							if(rows2[0].editor.personId==loginId || roles.indexOf('jtxtgly')!=-1){
								parent.$('#child-win').window({
									...
									title:"修改记录",
									cache:false,
									content:'<iframe src="${pageContext.request.contextPath}/shenhua/check/periodical_check_inconformity_update" frameborder="0" width="100%" height="100%"/>'
								});
							}else{
					...
					if(resources.indexOf('0106020505')==-1){
						...
						if(loginId==editor || roles.indexOf('jtxtgly')!=-1){
							parent.$('#child-win').window({
						 		...
						 		title:'添加不符合项-不安全行为',
						 		cache:false,
						 		content:'<iframe src="${pageContext.request.contextPath}/shenhua/check/periodical_check_inconformity_add_unsafeAct" frameborder="0" width="100%" height="100%"/>'
						 	});
						}else{
