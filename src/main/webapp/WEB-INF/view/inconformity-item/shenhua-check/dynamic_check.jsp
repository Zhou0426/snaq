...
<script type="text/javascript">
	...
	$(function(){
		...
		$('#dg').datagrid({
			...
			queryParams:{checkType:'动态检查',checkerFrom:'神华'},//请求远程数据发送额外的参数
			...
			onSelect:function(index,row){
				if(resources.indexOf('01060101')==-1){
					$('#unsafecondition-add').css('display','none');
				}
				if(resources.indexOf('01060102')==-1){
				...
				if(resources.indexOf('01060103')==-1){
				...
				handler:function(){
					...
					if(resources.indexOf('01060101')==-1){
						...
						$('#win').window({
			 				...
			 				title:'添加不符合项-隐患',
			 				cache:false,
			 				content:'<iframe src="${pageContext.request.contextPath}/shenhua/check/dynamic_check_add" frameborder="0" width="100%" height="100%"/>'
			 			});
					}
				}
			},{
						...
						if(rows.length==1){
							...
							if(loginId==rows[0].editor.personId || roles.indexOf('jtxtgly')!=-1){
								$('#win').window({
									...
									title:"修改记录",
									cache:false,
									content:'<iframe src="${pageContext.request.contextPath}/shenhua/check/dynamic_check_update" frameborder="0" width="100%" height="100%"/>'
		...
		$('#dg2').datagrid({
			...
			idField:"id",// id字段为标识字段,那么此字段状态将会被dg记录下来
			//title:"动态检查",
			queryParams:{checkTypeSn:'0',checkerFromSn:'4'},//请求远程数据发送额外的参数
			fitColumns:true,/*列宽自动*/
			fit:true,
			striped:true,/*斑马线效果  */
			nowrap:false,/*数据同一行*/
					...
					if(resources.indexOf('01060105')==-1){
						$('#unsafeact-add').css('display','none');
					 	...
					 	$('#win2').window({
					 		...
					 		title:'不符合项-不安全行为',
					 		cache:false,
					 		content:'<iframe src="${pageContext.request.contextPath}/shenhua/check/dynamic_check_add_unsafeAct" frameborder="0" width="100%" height="100%"/>'
					 	});
					}
				}
			},{
