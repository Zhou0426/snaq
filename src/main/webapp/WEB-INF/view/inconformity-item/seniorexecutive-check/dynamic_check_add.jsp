	...
	$(function(){
		...
		$('#tb').textbox({    
    		...
    		iconCls:'icon-man', 
   		 	...
   		 	onChange:function(newValue, oldValue){
   		 		if(newValue!=""){
   		 			$('#cc7').combogrid('grid').datagrid('load',{'q':newValue,'departmentSn':'1'});
   		 		}
   		 	}
		});
		//下拉面板
		...
		$("#submit").click(function(){
			...
			if($("#ff").form("validate")){
				...
				$("#ff").form("submit",{
					...
					queryParams:{'personId':principalId,'ids':personIds,isConfirm:"false",'methods':methods,'checkType':'动态检查','checkerFrom':'高层管理人员'},
					success:function(data){
						...
						if(status=="ok"){
							...
							$.messager.confirm('确认对话框', msg, function(r){
								if (r){
									$('#ff').form('submit', {
										url:'${pageContext.request.contextPath}/inconformity/item/unsafeconditionAction_save.action',
										queryParams:{'personId':principalId,'ids':personIds,isConfirm:"true",'methods':methods,'checkType':'动态检查','checkerFrom':'高层管理人员'},
