...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#d-i").datagrid({
			...
			onSelect:function(index,row){
				if(loginId==editor || $.inArray(loginId,checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
					if(resources.indexOf('0105030501')==-1){
						$('#unsafecondition-add').css('display','none');
					}
					if(resources.indexOf('0105030502')==-1){
					...
					if(resources.indexOf('0105030503')==-1){
			...
			toolbar:[{
					...
					if(resources.indexOf('0105030501')==-1){
						...
						if(loginId==editor || $.inArray(loginId, checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
							parent.$('#child-win').window({
				 				...
				 				title:'添加不符合项-隐患',
				 				cache:false,
				 				content:'<iframe src="${pageContext.request.contextPath}/seniorexecutive/check/special_check_inconformity_add" frameborder="0" width="100%" height="100%"/>'
				 			});
						}else{
				...
				handler:function(){
					if(resources.indexOf('0105030503')==-1){
						...
						if(rows2.length==1){
							//判断是否有权限
							if(rows2[0].editor.personId==loginId || roles.indexOf('jtxtgly')!=-1){
								parent.$('#child-win').window({
									width:730,
									height:300,
									title:"修改记录",
									cache:false,
									content:'<iframe src="${pageContext.request.contextPath}/seniorexecutive/check/special_check_inconformity_update" frameborder="0" width="100%" height="100%"/>'
								});
							}else{
		...
		$('#dg2').datagrid({
			...
			onSelect:function(index,row){
				if(loginId==editor || $.inArray(loginId, checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
					if(resources.indexOf('0105030505')==-1){
					...
					if(row.editorId==loginId || $.inArray('${sessionScope.pId}',row.checkersId.split(','))!=-1 || roles.indexOf('jtxtgly')!=-1){
						if(resources.indexOf('0105030506')==-1){
							$('#unsafeact-remove').css('display','none');
						}else{
							$('#unsafeact-remove').css('display','');
						}
						if(resources.indexOf('0105030507')==-1){
					...
					if(resources.indexOf('0105030505')==-1){
						...
						if(loginId==editor || $.inArray(loginId, checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
							parent.$('#child-win').window({
						 		...
						 		title:'添加不符合项-不安全行为',
						 		cache:false,
						 		content:'<iframe src="${pageContext.request.contextPath}/seniorexecutive/check/special_check_inconformity_add_unsafeAct" frameborder="0" width="100%" height="100%"/>'
						 	});
						}else{
