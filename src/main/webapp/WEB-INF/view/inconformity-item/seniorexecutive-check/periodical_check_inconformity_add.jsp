...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#submit").click(function(){
			...
			if($("#ff").form("validate")){
				...
				$("#ff").form("submit",{
					...
					queryParams:{'personId':principalId,'ids':personIds,isConfirm:"false",'periodicalCheckSn':rows[0].periodicalCheckSn,'methods':methods,'checkType':'定期检查','checkerFrom':'高层管理人员'},
					success:function(data){
						...
						if(status=="ok"){
							...
							$.messager.confirm('确认对话框', msg, function(r){
								if (r){
									$('#ff').form('submit', {
										url:'${pageContext.request.contextPath}/inconformity/item/unsafeconditionAction_save.action',
										queryParams:{'personId':principalId,'ids':personIds,isConfirm:"true",'periodicalCheckSn':rows[0].periodicalCheckSn,'methods':methods,'checkType':'定期检查','checkerFrom':'高层管理人员'},
		...
		$('#p').panel({    
  			...
  			title:'添加详细',
  			onCollapse:function(){
  				parent.$('#child-win').window('resize',{
  					width: 730,
  					height: 300
  				});  								
  	  		},
