...
<script type="text/javascript">
	...
	$(function(){
		...
		$.ajax({
            ...
            data: {'departmentSn':rows[0].checkedDepartment.departmentSn},
            dataType: "json",
            ...
            if($('#j3').css('display')=="none"){
            	 $('#cc10').combotree({
                    url:'${pageContext.request.contextPath}/inconformity/item/inconformityItemAction_indexTree.action',
                 	editable:false,
     				queryParams:{'departmentSn':rows[0].checkedDepartment.departmentSn},
     				panelHeight:250,
     				prompt:'下拉选择标准！',
		...
		$("textarea",$("#text").next("span")).blur(function(){
			...
			$('#cc3').combogrid('grid').datagrid('reload',{'q':$('#cc3').combogrid('getText'),'text':$('#text').val(),'departmentSn':rows[0].checkedDepartment.departmentSn});
		});	
		$('#cc3').combogrid({          
    		idField:'id',    
   	 		textField:'indexName',    
		...
		//不符合项专业
		$('#cc5').combobox({    
			url:'${pageContext.request.contextPath}/inconformity/item/inconformityItemAction_queryByDepartmentSn.action',
			queryParams:{'departmentSn':rows[0].checkedDepartment.departmentSn},
			panelHeight:250,
		...
		$("#cc6").textbox({
			readonly:true,
			value:rows[0].checkedDepartment.departmentName
		});
		//检查人员回显
    		...
    		iconCls:'icon-man', 
   		 	...
   		 	onChange:function(newValue, oldValue){
   		 		if(newValue!=""){
   		 			...
   		 			$('#cc7').combogrid('grid').datagrid('load',{q:newValue,advancedSearch:false,'departmentSn':rows[0].checkedDepartment.departmentSn});
   		 		}
   		 	}
		});
		//数据回显
		...
		$('#reset').click(function(){
			...
			$("#cc6").textbox('setValue',rows[0].checkedDepartment.departmentName);
		});
		$('#cc8').textbox({
			value:principalName,
			editable:false
		...
		$("#submit").click(function(){
			...
			if($("#ff").form("validate")){
				...
				$("#ff").form("submit",{
					...
					queryParams:{'personId':principalId,'ids':personIds,'methods':methods,'inconformityItemid':rows[0].id,'departmentSn':rows[0].checkedDepartment.departmentSn},
					success:function(data){
	...
	<form id="ff" method="post">
		<table>
			<tr>
				...
				<td>
					<input id="cc6" style="width: 200px;" class="easyui-textbox">
				</td>
			...
			<tr>
				...
				<td colspan="3">
					<input name="indexId" id="cc3" class="easyui-combogrid" style="width: 530px;"/>
				</td>
			</tr>
			<tr>
				<td></td>
