	...
	function standard(index){
		$('#dg').datagrid('clearSelections');//清处多选的行  
 		$('#win').window({
 			...
 			title:'相关指标',
 			cache:false,
 			content:'<iframe src="${pageContext.request.contextPath}/standard/template_standardindex" frameborder="0" width="100%" height="100%"/>'
 		});
	};
	function details(){
		$('#dg').datagrid('clearSelections');//清处多选的行  
 		$('#win').window({
 			...
 			title:'模板附件',
 			cache:false,
 			content:'<iframe src="${pageContext.request.contextPath}/standard/template_attachment" frameborder="0" width="100%" height="100%"/>'
 		});
	};
	$(function(){
		...
		$('#dg').datagrid({
			url:'standard/documentTemplateAction_query.action',//要改为一个action
				...
				handler:function(){
					if(roles.indexOf('jtxtgly')==-1){
						...
						$('#win').window({
			 				...
			 				title:'添加',
			 				cache:false,
			 				content:'<iframe src="${pageContext.request.contextPath}/standard/template_add" frameborder="0" width="100%" height="100%"/>'
			 			});
					}
				}
			},{
						...
						if(rows.length==0){
							...
							$.messager.confirm('删除确认','您确定要删除选中的数据吗？',function(r){
								if(r){
									...
									//4、发送请求提交删除信息
			                        $.post("standard/documentTemplateAction_deleteByIds.action",{ids:ids},function(result){					
				...
				handler:function(){
					if(roles.indexOf('jtxtgly')==-1){
						...
						if(rows.length==1){						
							...
							title:"修改记录",
							cache:false,
							content:'<iframe src="${pageContext.request.contextPath}/standard/template_update" frameborder="0" width="100%" height="100%"/>'
							});
						}else{
							$.messager.show({
								title:'我的提示',
			...
			columns:[[
				...
				{field:'attachments',title:'模板附件',formatter:function(value,row,index){
					return "<a href='#' onclick='details()' style='text-decoration:none;'>" + "详情"+"["+value+"]" + "</a>";
				}},
				{field:'standardindex',title:'相关指标',formatter:function(value,row,index){
					return "<a href='#' onclick='standard("+index+")' style='text-decoration:none;'>" + "相关指标"+"["+value+"]" + "</a>";
				}}
				]]
