...
<script type="text/javascript">
	$(function(){
		...
		//数据回显
		var rows=parent.$("#st").datagrid("getSelections");
		//对表单数据进行填充
		$('#ff').form('load',{
			id:rows[0].id,
			standardSn:rows[0].standardSn,
			standardName:rows[0].standardName,
			standardType:rows[0].standardType,
		...
		$("#submit").click(function(){
			...
			if($("#ff").form("validate")){
				//ajax
				$('#ff').form("submit",{
					url:'standard/standardAction_updateStandard.action',
