	...
	$(function(){
		...
		$.ajax({
            ...
            success: function(data){
                	$('[name=departmentSn]').val(data.departmentSn);
                    $('#cc1').textbox({
		...
		$('#cc2').combogrid({
		    ...
		    mode:'remote',
		    prompt:'输入姓名或编号搜索人员',
		    queryParams:{'departmentSn':$('[name=departmentSn]').val()},
    		columns:[[    
		...
		$('#tb').textbox({    
    		...
    		iconCls:'icon-man', 
   		 	...
   		 	onChange:function(newValue, oldValue){
   		 		if(newValue!=""){
   		 			$('#cc3').combogrid('grid').datagrid('load',{'q':newValue,'departmentSn':$('[name=departmentSn]').val()});
   		 		}
   		 	}
		});
		//ajax提交
		$('#submit').click(function(){
			...
			if($('#ff').form('validate')){
				$('#ff').form('submit',{
					...
					queryParams:{'ids':personIds,'standardSn':standardSn,'systemAuditType':type},
					success:function(data){
						var data = eval('(' + data + ')');  
						if(data.status=="ok"){
							...
							parent.$.messager.alert("提示信息",'当前单位本月审核已经录入，无需重复录入！');
						}else{
...
<body>
	...
	<input name="editorId" type="hidden" value="${personId}"/>
	<input name="departmentSn" type="hidden">
		<table>
        	<tr>
