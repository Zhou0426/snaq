...
<script type="text/javascript">
	...
	$(function(){
		...
		var auditors="";
		var auditTeamLeader="";
		var content1='<iframe src="${pageContext.request.contextPath}/system/audit/audit_unsafecondition_add" frameborder="0" width="100%" height="100%"/>';
		var content2='<iframe src="${pageContext.request.contextPath}/system/audit/audit_unsafecondition_update" frameborder="0" width="100%" height="100%"/>';
		var content3='<iframe src="${pageContext.request.contextPath}/system/audit/audit_unsafeact_add" frameborder="0" width="100%" height="100%"/>';
		var content4='<iframe src="${pageContext.request.contextPath}/system/audit/audit_unsafeact_update" frameborder="0" width="100%" height="100%"/>';
		if(type=="单位审核"){
			role="dwxtgly";
			addresource="08010701";
			deleteresource="08010702";
			updateresource="08010703";
			unsafeActAdd="08010705";
			unsafeActRemove="08010706";
			unsafeActUpdate="08010707";
			editor=row[0].editor.personId;
			auditTeamLeader=row[0].auditTeamLeader.personId;
			auditors=row[0].auditors.personIds.split(',');
		}else if(type=="季度审核"){
			role="jtxtgly";
			addresource="08020701";
			deleteresource="08020702";
			updateresource="08020703";
			unsafeActAdd="08020705";
			unsafeActRemove="08020706";
			unsafeActUpdate="08020707";
			editor=row[0].editor.personId;
			auditTeamLeader=row[0].auditTeamLeader.personId;
			auditors=row[0].auditors.personIds.split(',');
		}else{
			role="jtxtgly";
			addresource="08030701";
			deleteresource="08030702";
			updateresource="08030703";
			unsafeActAdd="08030705";
			unsafeActRemove="08030706";
			unsafeActUpdate="08030707";
			editor=row[0].editor.personId;
			content1='<iframe src="${pageContext.request.contextPath}/system/audit/shenhua_unsafecondition_add" frameborder="0" width="100%" height="100%"/>';
			content2='<iframe src="${pageContext.request.contextPath}/system/audit/shenhua_unsafecondition_update" frameborder="0" width="100%" height="100%"/>';
			content3='<iframe src="${pageContext.request.contextPath}/system/audit/shenhua_unsafeact_add" frameborder="0" width="100%" height="100%"/>';
			content4='<iframe src="${pageContext.request.contextPath}/system/audit/shenhua_unsafeact_update" frameborder="0" width="100%" height="100%"/>';
		}
		$("#d-i").datagrid({
			url:'${pageContext.request.contextPath}/inconformity/item/unsafeconditionAction_query',
			idField:"id",// id字段为标识字段,那么此字段状态将会被dg记录下来
			queryParams:{'auditSn':row[0].auditSn},//请求远程数据发送额外的参数
			...
			onSelect:function(index,row){
				//第二层权限限制
				if(resources.indexOf(addresource)==-1){
					$('#unsafecondition-add').css('display','none');
				}else{
					if(loginId==editor || $.inArray(loginId, auditors)!=-1 || loginId==auditTeamLeader || roles.indexOf(role)!=-1){
						$('#unsafecondition-add').css('display','');
					}else{
						$('#unsafecondition-add').css('display','none');
					}
				}
				if(resources.indexOf(deleteresource)==-1){
					$('#unsafecondition-delete').css('display','none');
				}else{
					if((row.editor.isnull==false&&loginId==row.editor.personId) || roles.indexOf(role)!=-1){
						$('#unsafecondition-delete').css('display','');
					}else{
						$('#unsafecondition-delete').css('display','none');
					}
				}
				if(resources.indexOf(updateresource)==-1){
			...
			toolbar:[{
				...
				handler:function(){
					if(resources.indexOf(addresource)==-1){
						...
						if(loginId==editor || $.inArray(loginId, auditors)!=-1 || loginId==auditTeamLeader || roles.indexOf(role)!=-1){
							...
							$.messager.alert('我的消息','对不起！您没有在此添加隐患的权限！','warning');
						}
					}
				}
			},{
				...
				handler:function(){
					if(resources.indexOf(deleteresource)==-1){
						...
						if(rows.length==0){
							...
							//权限判断
							if((rows[0].editor.isnull==false&&loginId==rows[0].editor.personId) || roles.indexOf(role)!=-1){
				...
				handler:function(){
					if(resources.indexOf(updateresource)==-1){
						...
						if(rows2.length==1){
							//判断是否有权限
							if((rows2[0].editor.isnull==false&&loginId==rows2[0].editor.personId) || roles.indexOf(role)!=-1){
								parent.$('#child-win').window({
									width:740,
									height:390,
									title:"修改记录",
				...
				handler:function(){
					$.messager.confirm('导出确认','您确定要导出当前数据吗？',function(r){
						if(r){
							...
							form.attr("action","${pageContext.request.contextPath}/system/audit/systemauditAction_exportUnsafeCondition");
							//添加input
												
							var input1=$("<input>");
							input1.attr("type","hidden");
							input1.attr("name","auditSn");
							input1.attr("value",row[0].auditSn);
							//将表单放入body
							$("body").append(form);
							form.append(input1);
							form.submit();//提交表单
			...
			columns:[[
				...
				{field:'inconformityLevel',title:'不符合项等级',width:'9%',styler: function(value,row,index){
	  				if(value=="严重不符合项"){
		        	  	return 'background-color:red';
					}else if(value=="一般不符合项"){
				...
				{field:'correctProposal',width:200,title:'整改建议',width:'30%'},
		...
		$('#dg2').datagrid({
			...
			onSelect:function(index,row){
				...
				if(resources.indexOf(unsafeActAdd)==-1){
					$('#unsafeact-add').css('display','none');
				}else{
					if(loginId==editor || $.inArray(loginId, auditors)!=-1 || loginId==auditTeamLeader || roles.indexOf(role)!=-1){
						$('#unsafeact-add').css('display','');
					}else{
						$('#unsafeact-add').css('display','none');
					}
				}
				
				if(resources.indexOf(unsafeActRemove)==-1){
					$('#unsafeact-remove').css('display','none');
				}else{
					if((row.editor!="无"&&loginId==row.editorId) || roles.indexOf(role)!=-1){
				...
				if(resources.indexOf(unsafeActUpdate)==-1){
				...
				handler:function(){
					if(resources.indexOf(unsafeActAdd)==-1){
						...
						if(loginId==editor || $.inArray(loginId, auditors)!=-1 || loginId==auditTeamLeader || roles.indexOf(role)!=-1){
							...
							$.messager.alert('我的消息','对不起！您没有添加此记录的权限！','warning');
						}
					}
				}
			},{
				...
				handler:function(){
					if(resources.indexOf(unsafeActRemove)==-1){
						...
						if(rows.length==0){
							...
							if((rows[0].editor!="无"&&loginId==rows[0].editorId) || roles.indexOf(role)!=-1){
								$.messager.confirm('删除确认','您确定要删除选中的数据吗？',function(r){
									if(r){
										var ids="";
										for(var i=0;i<rows.length;i++){
			...
			columns:[[
				...
				{field:'violator',width:50,title:'不安全行为人员'},
		...
		$('#unsafecondition-export').linkbutton({plain:false});
...
<body>
	<div id="tt" class="easyui-tabs" data-options="fit:true" style="margin:1px;">   
    	<div title="隐患"  style="display:none;margin:1px;">   
        	<table id="d-i"></table>   
   		</div>   
    	 <div title="不安全行为" style="display:none;margin:1px;"> 
    		<table id="dg2"></table>
