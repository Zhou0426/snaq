...
<script type="text/javascript">
	...
	function deleteit(index){
		if(powers==true){
			...
			$.messager.confirm('删除确认','您确定要删除选中的数据吗？',function(r){
				if(r){
					//发送请求提交删除信息
	                $.post("${pageContext.request.contextPath}/system/audit/systemAuditAttachmentAction_delete.action",{'id':id},function(result){
	                	//删除页面，刷新
	...
	$(function(){
		var parentrow=parent.$("#dg").datagrid("getChecked")[0];
		var auditSn=parentrow.auditSn;
		//判断权限
		...
		if(type=="单位审核"){
			...
			auditTeamLeader=parentrow.auditTeamLeader.personId;
			auditors=parentrow.auditors.personIds.split(',');
		}else if(type=="季度审核"){
			role="jtxtgly";
			editor=parentrow.editor.personId;
			auditTeamLeader=parentrow.auditTeamLeader.personId;
		...
		$("#dg").datagrid({
		    url:'${pageContext.request.contextPath}/system/audit/systemAuditAttachmentAction_query',
			idField:"id",// id字段为标识字段,那么此字段状态将会被dg记录下来
			queryParams:{'auditSn':auditSn},//请求远程数据发送额外的参数
			fit:true,
			striped:true,/*斑马线效果  */
			nowrap:true,/*数据同一行*/
			loadmsg:'请等待',
			...
			toolbar:[{
				id:'text',
				text:"<form id='upload' method='post' enctype='multipart/form-data'><input id='fb' type='text' name='fileModel.upload' style='width:270px'/><input name='auditSn' type='hidden'/><a id='submit' href='#'>提交</a></form>"				 
			}],
		    columns:[[ 
				{field:'id',title:'逻辑主键',hidden:true},
				{field:'attachmentType',title:'附件类型',width:'20%'},
		        {field:'logicalFileName',title:'附件逻辑路径',width:'58%',formatter:function(value,row,index){
		        ...
		        {field:'op',title:'操作',width:'18%',formatter:function(value,row,index){
		        	if(powers==true){
		...
		$("#submit").click(function(){
			if(powers==true){
				...
				if($("#upload").form("validate")){
					//ajax提交
					$('#upload').form('submit',{
						url:'${pageContext.request.contextPath}/system/audit/systemAuditAttachmentAction_attachmentUpload.action',
						success:function(){
							$("#upload").form("disableValidation");
							$("#upload").form("reset");
							$("#dg").datagrid("reload");
