	...
	$(function(){
		$('#dg').datagrid({
			...
			onLoadSuccess:function(){
				...
				$('[name="month"]').click(function(e){
					...
					$('#win').window({
			 			...
			 			title:' ',
			 			cache:false,
			 			content:'<iframe src="${pageContext.request.contextPath}/security/appraisals/unitmonth" frameborder="0" width="100%" height="100%"/>'
			 		});
				});
				//季度点击
				$('[name="audit"]').click(function(e){
					$('#value').attr('value',$(e.target).attr('value'));
					$('#dg').datagrid('clearSelections');
					$('#win').window({
			 			width:600,
			 			...
			 			title:' ',
			 			cache:false,
			 			content:'<iframe src="${pageContext.request.contextPath}/security/appraisals/unitquarter" frameborder="0" width="100%" height="100%"/>'
			 		});
				});
			},
			...
			toolbar:[{
				...
				handler:function(){
					$.messager.confirm('导出确认','您确定要导出当前数据吗？',function(r){
						if(r){
							var titles="部门名称,一月,二月,三月,第一季度审核得分,第一季度综合得分,四月,五月,六月,第二季度审核得分,第二季度综合得分,七月,八月,九月,第三季度审核得分,第三季度综合得分,十月,十一月,十二月,第四季度审核得分,第四季度综合得分,年度,"+sortValue;
							//向后台发送请求
							var form=$("<form>");
			...
			columns:[[
				...
				{field:'audit1',title:'第一季度审核得分',sortable:true,align:'center',formatter:function(value,index,row){
					if(value!=null && value>0){
						return '<a href="#" name="audit" value="1" style="text-decoration:none">'+(value).toFixed(2)+'</a>';
					}else{
						return "--";
					}
				}},
				{field:'season1',title:'第一季度综合得分',sortable:true,align:'center',formatter:function(value,index,row){
					if(value!=null && value>0){
				...
				{field:'audit2',title:'第二季度审核得分',sortable:true,align:'center',formatter:function(value,index,row){
					if(value!=null && value>0){
						return '<a href="#" name="audit" value="2" style="text-decoration:none">'+(value).toFixed(2)+'</a>';
					}else{
						return "--";
					}
				}},
				{field:'season2',title:'第二季度综合得分',sortable:true,align:'center',formatter:function(value,index,row){
					if(value!=null && value>0){
						return (value).toFixed(2);
					}else{
						return "--";
				...
				{field:'audit3',title:'第三季度审核得分',sortable:true,align:'center',formatter:function(value,index,row){
					if(value!=null && value>0){
						return '<a href="#" name="audit" value="3" style="text-decoration:none">'+(value).toFixed(2)+'</a>';
					}else{
						return "--";
					}
				}},
				{field:'season3',title:'第三季度综合得分',sortable:true,align:'center',formatter:function(value,index,row){
					if(value!=null && value>0){
						return (value).toFixed(2);
					}else{
						return "--";
				...
				{field:'audit4',title:'第四季度审核得分',sortable:true,align:'center',formatter:function(value,index,row){
					if(value!=null && value>0){
						return '<a href="#" name="audit" value="4" style="text-decoration:none">'+(value).toFixed(2)+'</a>';
					}else{
						return "--";
					}
				}},
				{field:'season4',title:'第四季度综合得分',sortable:true,align:'center',formatter:function(value,index,row){
					if(value!=null && value>0){
						return (value).toFixed(2);
					}else{
						return "--";
