			...
			var row = $('#standard').treegrid('getSelected');
			    if (row!=null){  
					$('#standard-win').window({
						...
						title:'危险源',
						cache:false,
						content:'<iframe src="${pageContext.request.contextPath}/standard/standardindex_hazard"+ frameborder="0" width="100%" height="100%"/>'
					});
				}
		}
	$(function(){
		...
		$('#standard').treegrid({    
		    ...
		    columns:[[
		        ...
		        {field:'indexSn',title:'指标编号',sortable:true,hidden:true},
		        ...
		        {field:'speciality',title:'专业',formatter:function(value,row){
			        if(row.speciality.num>0){
			        	return row.speciality.specialityNames;
			        }
		        	
		        }},
		        {field:'hazard',title:'危险源',formatter:function(value,row){
		        	return "<a href='#' onclick='hazard("+row.id+")' style='text-decoration:none'>" + "危险源"+"["+value+"]" + "</a>";
		        }},	
		        {field:'auditMethod',title:'审核方法',formatter:function(value,row){
		        	return "<a href='#' onclick='auditmethod("+row.id+")' style='text-decoration:none'>" + "审核方法"+"["+value+"]" + "</a>";
		        }},	        
		        {field:'document',title:'模板详情',formatter:function(value,row){
		        	return "<a href='#' onclick='muban("+row.id+")' style='text-decoration:none'>" + "模板详情"+"["+value+"]" + "</a>";
		        }}
		    ]]    
		}); 
  		//下拉列表框
		...
		//添加
		$('#add').click(function(){
			if(resources.indexOf('070201')==-1 || roles.indexOf('jtxtgly')==-1){
				...
				if($('#cc2').combobox('getValue')==""){
	 				...
	 				title:'添加',
	 				cache:false,
	 				content:'<iframe src="${pageContext.request.contextPath}/standard/standardindex_add" frameborder="0" width="100%" height="100%"/>'
		 			});
		 		}
			}
		...
		$('#delete').click(function(){
			if(resources.indexOf('070202')==-1 || roles.indexOf('jtxtgly')==-1){
		...
		$('#update').click(function(){
			if(resources.indexOf('070203')==-1 || roles.indexOf('jtxtgly')==-1){
					...
					$.post("",null,function(result){
						if(result=="fail"){
							parent.$.messager.confirm('权限不足提示','权限不足，是否注销重新登录？',function(r){
								...
								title:"修改记录",
								cache:false,
								content:'<iframe src="${pageContext.request.contextPath}/standard/standardindex_update" frameborder="0" width="100%" height="100%"/>'
		...
		$('#import').click(function(){
			if(resources.indexOf('070204')==-1 || roles.indexOf('jtxtgly')==-1){
				...
				if(standardSn.length>0){
					$.messager.confirm('导出确认','您确定要导出当前数据吗？',function(r){
						if(r){
							...
							input5.attr("value","standardindex");
		...
		//导入关联危险源
		$('#importh').click(function(){
			if(resources.indexOf('070206')==-1 || roles.indexOf('jtxtgly')==-1){
				$('#importh').css('display','none');
			}else{
				$("#standard-win").window({
					width:360,
					height:150,
					title:"Excel导入",
					cache:false,
					content:'<iframe src="${pageContext.request.contextPath}/standard/index_hazard_excel" frameborder="0" width="100%" height="100%"/>'						
				});
			}
		});
		//导出关联危险源
		$('#exporth').click(function(){
			if(resources.indexOf('070207')==-1){
				$('#exporth').css('display','none');
			}else{
				var standardSn=$('#cc2').combobox('getValue');
				if(standardSn.length>0){
					$.messager.confirm('导出确认','您确定要导出当前数据吗？',function(r){
						if(r){
							...
							form.attr("target","");
							form.attr("method","post");
							form.attr("action","${pageContext.request.contextPath}/hazard/download_exportHazardExcelStandardIndex2.action");
							//添加input
												
							var input1=$("<input>");
							input1.attr("type","hidden");
							input1.attr("name","standardSn");
							input1.attr("value",standardSn);
							form.append(input1);
							form.submit();//提交表单
							$('#dd').dialog('open');
							timerId=setInterval(getForm,500);
...
<body style="margin: 1px;">
    ...
    <div id="tb">
		...
		<a id="import" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-excel',plain:false">导入评分标准</a>
		<a id="export" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-excel',plain:false">导出评分标准</a>
		<a id="importh" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-excel',plain:false">导入关联危险源</a>
		<a id="exporth" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-excel',plain:false">导出关联危险源</a>			
	</div>
    <!-- 导出进度条 -->
    <div id="dd">
