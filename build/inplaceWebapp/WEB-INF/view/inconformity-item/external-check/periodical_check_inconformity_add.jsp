		...
		$("#submit").click(function(){
			...
			if($("#ff").form("validate")){
				...
				$("#ff").form("submit",{
					...
					queryParams:{'personId':principalId,isConfirm:"false",'periodicalCheckSn':rows[0].periodicalCheckSn,'methods':methods,'checkType':'定期检查','checkerFrom':'外部'},
					success:function(data){
						...
						if(status=="ok"){
							...
							$.messager.confirm('确认对话框', msg, function(r){
								if (r){
									$('#ff').form('submit', {
										url:'${pageContext.request.contextPath}/inconformity/item/unsafeconditionAction_save.action',
										queryParams:{'personId':principalId,isConfirm:"true",'periodicalCheckSn':rows[0].periodicalCheckSn,'methods':methods,'checkType':'定期检查','checkerFrom':'外部'},
