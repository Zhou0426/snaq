...
<script type="text/javascript">
	...
	$(function(){
		...
		$("textarea",$("#text").next("span")).blur(function(){
			$('#cc3').combogrid('grid').datagrid('reload',{'q':$('#cc3').combogrid('getText'),'text':$('#text').val(),'departmentSn':departmentSn});
		});	
		$('#cc3').combogrid({          
    		idField:'id',    
   	 		textField:'indexName',    
		...
		$('#tb').textbox({    
    		...
    		iconCls:'icon-man', 
   		 	...
   		 	onChange:function(newValue, oldValue){
   		 		if(newValue!=""){
   		 			$('#cc7').combogrid('grid').datagrid('load',{'q':newValue,'departmentSn':departmentSn});
   		 		}
   		 	}
		});
		//设置默认值
		...
		$('#reset').click(function(){
			...
			$("#cc6").textbox('setValue',departmentName);
			$('#cc8').textbox({
				value:principalName,
				editable:false
			});
		...
		$("#submit").click(function(){
			...
			if($("#ff").form("validate")){
				...
				$("#ff").form("submit",{
					...
					queryParams:{'personId':principalId,'ids':personIds,isConfirm:"false",'methods':methods,'checkType':'动态检查','checkerFrom':'区队','departmentSn':departmentSn},
					success:function(data){
						...
						if(status=="ok"){
							...
							$.messager.confirm('确认对话框', msg, function(r){
								if (r){
									$('#ff').form('submit', {
										url:'${pageContext.request.contextPath}/inconformity/item/unsafeconditionAction_save.action',
										queryParams:{'personId':principalId,'ids':personIds,isConfirm:"true",'methods':methods,'checkType':'动态检查','checkerFrom':'区队','departmentSn':departmentSn},
