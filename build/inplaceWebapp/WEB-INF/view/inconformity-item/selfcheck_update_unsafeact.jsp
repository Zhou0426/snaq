...
<script type="text/javascript">
	...
	$(function(){
		var a=1;
		var b=1;
		var rows=parent.$("#dg2").datagrid("getSelections");
...
// 		var department="";
// 		$("#cc6").combotree({
// 			url:'${pageContext.request.contextPath}/department/tree.action',
// 			required:true,
// 			editable:false,
// 			panelHeight:300,
...
// 		});
		...
		$('#violator').combogrid({    
			...
			onChange:function(newValue, oldValue){
				$('#violator').combogrid('grid').datagrid('load',{'q':newValue,'departmentSn':rows[0].checkedDepartmentSn});
			},
			onSelect:function(){
   		 	...
   		 	onChange:function(newValue, oldValue){
   		 		if(newValue!=""){
		 			...
		 			$('#cc7').combogrid('grid').datagrid('options').url=url;
   		 			$('#cc7').combogrid('grid').datagrid('load',{'q':newValue,'departmentSn':rows[0].checkedDepartmentSn});
   		 		}
   		 	}
		});
		$('#btn2').click(function(){
...
// 		}		
		...
		$("#cc6").textbox({
			readonly:true,
			value:rows[0].checkedDepartment
		});
		//检查人员回显
		var pSn=rows[0].checkersId;
		var pNa=rows[0].checkers;
		if($.trim(pNa)!=null && $.trim(pNa)!=""){
			var pIds=pSn.split(',');
			var pNas=pNa.split(',');
			...
			var url='${pageContext.request.contextPath}/inconformity/item/inconformityItemAction_getPersonByIds.action?ids='+pSn;
   		 	$('#cc7').combogrid('grid').datagrid('reload',url);
				...
				$("#ff").form("submit",{
					...
					success:function(data){
						...
						if(result=="login"){
							...
							parent.$.messager.alert("警告","修改异常，请重新操作！",'error');
							$("#ff").form("reset");
	 						parent.$("#win2").window("close");
	 						parent.$("#dg2").datagrid("reload"); 
						}
