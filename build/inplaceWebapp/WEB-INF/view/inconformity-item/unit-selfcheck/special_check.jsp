...
<script type="text/javascript">
	function inconformityItem(index){
		...
		$('#win').window({
			...
			title:'不符合项',
			cache:false,
			content:'<iframe src="${pageContext.request.contextPath}/unit/selfcheck/special_check_inconformityItem"+ frameborder="0" width="100%" height="100%"/>'
		});
	}
	...
	$(function(){
		...
		$('#dg').datagrid({
			url:'${pageContext.request.contextPath}/inconformity/item/specialAction_query.action',//要改为一个action
			idField:"id",// id字段为标识字段,那么此字段状态将会被dg记录下来
			title:"专项检查",
			queryParams:{checkerFrom:'单位'},//请求远程数据发送额外的参数
			...
			onSelect:function(index,row){
				if(resources.indexOf('01030301')==-1){
					$('#add').css('display','none');
				}
				if(resources.indexOf('01030302')==-1){
					$('#delete').css('display','none');
				}else{
					if(row.editor.personId==loginId || roles.indexOf('dwxtgly')!=-1){
						$('#delete').css('display','');
					}else{
						$('#delete').css('display','none');
					}
				}	
				if(resources.indexOf('01030303')==-1){
					...
					if(resources.indexOf('01030301')==-1){
						$('#add').css('display','none');
						 ...
						 $('#win').window({
				 				...
				 				title:'专项检查',
				 				cache:false,
				 				content:'<iframe src="${pageContext.request.contextPath}/unit/selfcheck/special_check_add" frameborder="0" width="100%" height="100%"/>'
				 		});
					}
				}
			},{
				...
				handler:function(){
					if(resources.indexOf('01030302')==-1){
						...
						if(rows.length==0){
							...
							if(rows[0].editor.personId==loginId || roles.indexOf('dwxtgly')!=-1){
								$.messager.confirm('删除确认','您确定要删除选中的数据吗？',function(r){
									if(r){
										...
										//4、发送请求提交删除信息
				                        $.post("${pageContext.request.contextPath}/inconformity/item/specialAction_deleteByIds.action",{ids:ids},function(result){
						...
						if(rows.length==1){
							...
							if(rows[0].editor.personId==loginId || roles.indexOf('dwxtgly')!=-1){
								$('#win').window({
									...
									title:"修改记录",
									cache:false,
									content:'<iframe src="${pageContext.request.contextPath}/unit/selfcheck/special_check_update" frameborder="0" width="100%" height="100%"/>'								
								});
							}else{
			...
			columns:[[
				...
				{field:'specialCheckSn',title:'专项检查编号'},
				{field:'specialCheckName',title:'专项检查名称'},
				...
				{field:'standardName',title:'依据评分标准',width:150,formatter:function(value,row,index){
				...
				{field:'specialities',title:'检查专业',width:200,formatter:function(value,row,index){
					if(row.specialities.num>0){
						return row.specialities.sName;
					}
				}},
...
<body style="margin: 1px">
	<!--头 -->
	<table id="dg"></table>
	<input id="type" type="hidden" value="special" />
