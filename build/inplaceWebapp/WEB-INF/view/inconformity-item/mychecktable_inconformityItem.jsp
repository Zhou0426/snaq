...
<script type="text/javascript">
	...
	$(function(){
		...
		$('#dg2').datagrid({
			...
			idField:"id",// id字段为标识字段,那么此字段状态将会被dg记录下来
			//title:"动态检查",
			queryParams:{'periodicalCheckSn':periodicalCheckSn,'specialCheckSn':specialCheckSn},//请求远程数据发送额外的参数
			fitColumns:true,/*列宽自动*/
			fit:true,
			striped:true,/*斑马线效果  */
			nowrap:false,/*数据同一行*/
			...
			toolbar:[{
				...
				handler:function(){
					parent.$('#child-win').window({
				 		...
				 		title:'添加不符合项-不安全行为',
				 		cache:false,
				 		content:'<iframe src="${pageContext.request.contextPath}/inconformity/item/my_unsafeact_add" frameborder="0" width="100%" height="100%"/>'
				 	});
				}
			},{
				id:'unsafeact-remove',
				...
				handler:function(){
					...
					if(rows.length==1){
						parent.$('#child-win').window({
							...
							title:"修改记录",
							cache:false,
							content:'<iframe src="${pageContext.request.contextPath}/inconformity/item/my_unsafeact_update" frameborder="0" width="100%" height="100%"/>'
						});
					}else{
						$.messager.show({
							title:'错误提示',
