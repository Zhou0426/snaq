		...
		$("#d-i").datagrid({
			...
			onSelect:function(index,row){
				if(loginId==editor || $.inArray(loginId,checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
					if(resources.indexOf('0106030501')==-1){
						$('#unsafecondition-add').css('display','none');
					}
					if(resources.indexOf('0106030502')==-1){
					...
					if(resources.indexOf('0106030503')==-1){
					...
					if(resources.indexOf('0106030501')==-1){
						...
						if(loginId==editor || $.inArray(loginId, checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
							parent.$('#child-win').window({
				 				...
				 				title:'添加不符合项-隐患',
				 				cache:false,
				 				content:'<iframe src="${pageContext.request.contextPath}/shenhua/check/special_check_inconformity_add" frameborder="0" width="100%" height="100%"/>'
				 			});
						}else{
						...
						if(rows2.length==1){
							...
							if(rows2[0].editor.personId==loginId || (rows2[0].checkers.num>0 && $.inArray('${sessionScope.pId}',rows2[0].checkers.personIds.split(','))!=-1) || roles.indexOf('jtxtgly')!=-1){
								parent.$('#child-win').window({
									...
									title:"修改记录",
									cache:false,
									content:'<iframe src="${pageContext.request.contextPath}/shenhua/check/special_check_inconformity_update" frameborder="0" width="100%" height="100%"/>'
								});
							}else{
					...
					if(resources.indexOf('0106030505')==-1){
						...
						if(loginId==editor || roles.indexOf('jtxtgly')!=-1){
							parent.$('#child-win').window({
						 		...
						 		title:'添加不符合项-不安全行为',
						 		cache:false,
						 		content:'<iframe src="${pageContext.request.contextPath}/shenhua/check/special_check_inconformity_add_unsafeAct" frameborder="0" width="100%" height="100%"/>'
						 	});
						}else{
							$.messager.alert('我的消息','对不起！您没有在此添加记录的权限！','warning');
						}
					}
				}
			},{
