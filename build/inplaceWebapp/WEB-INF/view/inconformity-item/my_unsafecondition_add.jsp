...
<script type="text/javascript">
	...
	$(function(){
		var checkdetail=parent.$('#checkdetail').val();
		var pIds="";
		var standardSn="";
		...
		if(checkdetail=="checkdetail"){
			var type=parent.parent.$('#details').val();
			var type1=parent.parent.$('#type').val();
			if(type=="my"){
				rows=parent.parent.$('#dg').datagrid('getSelections');
				
				standardSn=rows[0].standardSn;
				if(rows[0].checkers.num>0){
					 pIds=rows[0].checkers.pIds;
				}
			}else{
				if(type1=="季度审核" || type1=="单位审核"){
					rows=parent.parent.$('#dg').datagrid('getChecked');
					showMethod=false;
					standardSn=rows[0].standardSn;
					if(rows[0].auditors.num>0){
						 pIds=rows[0].auditors.Ids;
					}
				}else{
					rows=parent.parent.$('#dg').datagrid('getSelections');
					standardSn=rows[0].standard.standardSn;
					if(rows[0].checkers.num>0){
						 pIds=rows[0].checkers.personIds;
					}
				}
			}
			
			row=parent.$('#dg').datagrid('getSelections');
		}else{
			rows=parent.$('#dg').datagrid('getSelections');
		...
		var auditSn="";
		var checkType=null;
		var checkerFrom=null;
		if(rows[0].checkType=="定期检查"){
			checkType=rows[0].checkType;
			checkerFrom=rows[0].checkerFrom;
			periodicalCheckSn=rows[0].periodicalCheckSn;
		}else if(rows[0].checkType=="专项检查"){
			checkerFrom=rows[0].checkerFrom;
			checkType=rows[0].checkType;
            ...
            if($('#j3').css('display')=="none"){
            	 $('#cc10').combotree({
                    url:'${pageContext.request.contextPath}/inconformity/item/inconformityItemAction_indexTree.action',
                 	editable:false,
     				queryParams:{'standardSn':standardSn},
     				panelHeight:250,
     				prompt:'下拉选择标准！',
		...
		$("textarea",$("#text").next("span")).blur(function(){
			...
			$('#cc3').combogrid('grid').datagrid('reload',{'q':$('#cc3').combogrid('getText'),'text':$('#text').val(),'standardSn':standardSn});
		});	
		//对应标准
		$('#cc3').combogrid({          
    		idField:'id',    
		...
		//判断是否是
		if(checkdetail=="checkdetail"){
			$('#cc3').combogrid('setValue',{'id':row[0].id,'indexName':row[0].indexName});
			
        	if(row[0].isKeyIndex==true){
        		$("#cc9").combobox('setValue','严重不符合项');
        	}else{
        		$("#cc9").combobox('clear');
        	}
        	//专业
        	var data=$('#cc5').combobox('getData');
        	if(row[0].speciality.num>0){
        		var spSn=row[0].speciality.specialitySn.split(',');
        		$('#cc5').combobox({
        			formatter: function(row){
                			if($.inArray(row.specialitySn, spSn)!=-1){
                				return '<span style="color:red">' + row.specialityName + '</span>';
        	...
        	if(showMethod){
        		var url = '${pageContext.request.contextPath}/inconformity/item/inconformityItemAction_getHazard.action?indexId='+row[0].id;    
            	$('#cc4').combogrid('clear');
            	$('#cc4').combogrid('grid').datagrid('reload',url);
        		//审核方法
            	$('#cc1').combobox("clear");
            	methodnumber=0;
            	var url="${pageContext.request.contextPath}/standard/auditmethodAction_query?id="+row[0].id;
            	$('#cc1').combobox('reload', url);
        	}
		...
		$("#submit").click(function(){
			...
			if($("#ff").form("validate")){
				...
				$("#ff").form("submit",{
					...
					queryParams:{'personId':principalId,'ids':personIds,isConfirm:"false",'auditSn':auditSn,'specialCheckSn':specialCheckSn,'periodicalCheckSn':periodicalCheckSn,'methods':methods,"checkType":checkType,"checkerFrom":checkerFrom},
					success:function(data){
						...
						if(status=="ok"){
							...
							$.messager.confirm('确认对话框', msg, function(r){
								if (r){
									$('#ff').form('submit', {
										url:'${pageContext.request.contextPath}/inconformity/item/unsafeconditionAction_save.action',
										queryParams:{'personId':principalId,'ids':personIds,isConfirm:"true",'periodicalCheckSn':periodicalCheckSn,'auditSn':auditSn,'methods':methods,"checkType":checkType,"checkerFrom":checkerFrom},
	...
	<form method="post" id="ff">
		<table>
			...
			<tr>
				...
				<td>	
					<input id="cc2" name="inconformityItemNature" class="easyui-combobox" style="width: 200px;"/>				
				</td>
			...
			<tr>
				...
				<td colspan="3">
					<input id="text" name="problemDescription" class="easyui-textbox" style="width:530px; height:44px;"
						data-options="prompt:'上标使用：<sup>上标</sup>；下标使用：<sub>下标</sub>；'">
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td id="j3" colspan="2" style="display: none">
					<input id="cc10" style="width:320px;" class="easyui-combotree"/>
				</td>
			</tr>
			<!-- 审核方法 -->
			<tr id="scc1">
			...
			<tr>
				...
				<td>
					<input id="cc5" name="specialitySn" class="easyui-combobox" style="width: 200px;">  
				</td>
