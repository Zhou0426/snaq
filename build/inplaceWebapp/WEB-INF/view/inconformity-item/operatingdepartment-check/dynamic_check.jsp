...
<script type="text/javascript">
	...
	$(function(){
		...
		$('#dg').datagrid({
			...
			queryParams:{checkType:'动态检查',checkerFrom:'业务部门'},//请求远程数据发送额外的参数
			...
			onSelect:function(index,row){
				if(resources.indexOf('01040101')==-1){
					$('#unsafecondition-add').css('display','none');
				}
				if(resources.indexOf('01040102')==-1){
				...
				if(resources.indexOf('01040103')==-1){
				...
				handler:function(){
					...
					if(resources.indexOf('01040101')==-1){
						...
						$('#win').window({
			 				...
			 				title:'添加不符合项-隐患',
			 				cache:false,
			 				content:'<iframe src="${pageContext.request.contextPath}/operatingdepartment/check/dynamic_check_add" frameborder="0" width="100%" height="100%"/>'
			 			});
					}
				}
			},{
						...
						if(rows.length==1){
							...
							if(loginId==rows[0].editor.personId || (rows[0].checkers.num>0 && $.inArray('${sessionScope.pId}', rows[0].checkers.personIds.split(','))!=-1) || roles.indexOf('jtxtgly')!=-1){
								$('#win').window({
									...
									title:"修改记录",
									cache:false,
									content:'<iframe src="${pageContext.request.contextPath}/operatingdepartment/check/dynamic_check_update" frameborder="0" width="100%" height="100%"/>'							
								});
							}else{
		...
		$('#dg2').datagrid({
			...
			idField:"id",// id字段为标识字段,那么此字段状态将会被dg记录下来
			//title:"动态检查",
			queryParams:{checkTypeSn:'0',checkerFromSn:'2'},//请求远程数据发送额外的参数
			fitColumns:true,/*列宽自动*/
			fit:true,
			striped:true,/*斑马线效果  */
			nowrap:false,/*数据同一行*/
			...
			onSelect:function(index,row){
				if(resources.indexOf('01040105')==-1){
				...
				if(loginId==row.editorId || $.inArray('${sessionScope.pId}', row.checkersId.split(','))!=-1 || roles.indexOf('jtxtgly')!=-1){
					if(resources.indexOf('01040106')==-1){
						$('#unsafeact-remove').css('display','none');
					}else{
						$('#unsafeact-remove').css('display','');
					}
					if(resources.indexOf('01040107')==-1){
					...
					if(resources.indexOf('01040105')==-1){
						$('#unsafeact-add').css('display','none');
					 	...
					 	$('#win2').window({
					 		...
					 		title:'不符合项-不安全行为',
					 		cache:false,
					 		content:'<iframe src="${pageContext.request.contextPath}/operatingdepartment/check/dynamic_check_add_unsafeAct" frameborder="0" width="100%" height="100%"/>'
					 	});
					}
				}
			},{
