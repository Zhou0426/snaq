	...
	$(function(){
		...
		$("#d-i").datagrid({
			...
			onSelect:function(index,row){
				if(loginId==editor || $.inArray(loginId,checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
					if(resources.indexOf('0104030501')==-1){
						$('#unsafecondition-add').css('display','none');
					}
					if(resources.indexOf('0104030502')==-1){
						$('#unsafecondition-delete').css('display','none');
					}else{
						if(row.editor.personId==loginId || (row.checkers.num>0 && $.inArray('${sessionScope.pId}',row.checkers.personIds.split(','))!=-1) || roles.indexOf('jtxtgly')!=-1){
							$('#unsafecondition-delete').css('display','');
						}else{
							$('#unsafecondition-delete').css('display','none');
						}
					}
					if(resources.indexOf('0104030503')==-1){
			...
			toolbar:[{
					...
					if(resources.indexOf('0104030501')==-1){
						...
						if(loginId==editor || $.inArray(loginId, checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
							parent.$('#child-win').window({
				 				...
				 				title:'添加不符合项-隐患',
				 				cache:false,
				 				content:'<iframe src="${pageContext.request.contextPath}/operatingdepartment/check/special_check_inconformity_add" frameborder="0" width="100%" height="100%"/>'
				 			});
						}else{
				...
				handler:function(){
					if(resources.indexOf('0104030502')==-1){
						...
						if(rows.length==0){
							...
							if(rows[0].editor.personId==loginId || (rows[0].checkers.num>0 && $.inArray('${sessionScope.pId}',rows[0].checkers.personIds.split(','))!=-1) || roles.indexOf('jtxtgly')!=-1){
				...
				handler:function(){
					if(resources.indexOf('0104030503')==-1){
						...
						if(rows2.length==1){
							//判断是否有权限
							if(rows2[0].editor.personId==loginId || (rows2[0].checkers.num>0 && $.inArray('${sessionScope.pId}',rows2[0].checkers.personIds.split(','))!=-1) || roles.indexOf('jtxtgly')!=-1){
								parent.$('#child-win').window({
									width:730,
									height:410,
									title:"修改记录",
									cache:false,
									content:'<iframe src="${pageContext.request.contextPath}/operatingdepartment/check/special_check_inconformity_update" frameborder="0" width="100%" height="100%"/>'
								});
							}else{
		...
		$('#dg2').datagrid({
			...
			onSelect:function(index,row){
				if(loginId==editor || $.inArray(loginId, checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
					if(resources.indexOf('0104030505')==-1){
						$('#unsafeact-add').css('display','none');
					}
					if(row.editorId==loginId || $.inArray('${sessionScope.pId}',row.checkersId.split(','))!=-1 || roles.indexOf('jtxtgly')!=-1){
						if(resources.indexOf('0104030506')==-1){
							$('#unsafeact-remove').css('display','none');
						}else{
							$('#unsafeact-remove').css('display','');
						}
						if(resources.indexOf('0104030507')==-1){
					...
					if(resources.indexOf('0104030505')==-1){
						...
						if(loginId==editor || $.inArray(loginId, checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
							parent.$('#child-win').window({
						 		...
						 		title:'添加不符合项-不安全行为',
						 		cache:false,
						 		content:'<iframe src="${pageContext.request.contextPath}/operatingdepartment/check/special_check_inconformity_add_unsafeAct" frameborder="0" width="100%" height="100%"/>'
						 	});
						}else{
				...
				handler:function(){
					if(resources.indexOf('0104030506')==-1){
						...
						if(rows.length==0){
							...
							if(rows[0].editorId==loginId || $.inArray('${sessionScope.pId}',rows[0].checkersId.split(','))!=-1 || roles.indexOf('jtxtgly')!=-1){
								$.messager.confirm('删除确认','您确定要删除选中的数据吗？',function(r){
									if(r){
										var ids="";
										for(var i=0;i<rows.length;i++){
