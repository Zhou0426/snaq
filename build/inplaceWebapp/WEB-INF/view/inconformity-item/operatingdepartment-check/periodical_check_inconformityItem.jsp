...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#d-i").datagrid({
			...
			onSelect:function(index,row){
				if(loginId==editor || $.inArray(loginId, checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
					if(resources.indexOf('0104020501')==-1){
						$('#unsafecondition-add').css('display','none');
					}
					if(resources.indexOf('0104020502')==-1){
					...
					if(resources.indexOf('0104020503')==-1){
					...
					if(resources.indexOf('0104020501')==-1){
						...
						if(loginId==editor || $.inArray(loginId,checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
							parent.$('#child-win').window({
				 				...
				 				title:'添加不符合项-隐患',
				 				cache:false,
				 				content:'<iframe src="${pageContext.request.contextPath}/operatingdepartment/check/periodical_check_inconformity_add" frameborder="0" width="100%" height="100%"/>'
				 			});
						}else{
						...
						if(rows2.length==1){
							...
							if(rows2[0].editor.personId==loginId || (rows2[0].checkers.num>0 && $.inArray('${sessionScope.pId}',rows2[0].checkers.personIds.split(','))!=-1) || roles.indexOf('jtxtgly')!=-1){
								parent.$('#child-win').window({
									...
									title:"修改记录",
									cache:false,
									content:'<iframe src="${pageContext.request.contextPath}/operatingdepartment/check/periodical_check_inconformity_update" frameborder="0" width="100%" height="100%"/>'
		...
		$('#dg2').datagrid({
			...
			onSelect:function(index,row){
				if(loginId==editor || $.inArray(loginId, checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
					if(resources.indexOf('0104020505')==-1){
					...
					if(row.editorId==loginId || $.inArray('${sessionScope.pId}',row.checkersId.split(','))!=-1 || roles.indexOf('jtxtgly')!=-1){
						if(resources.indexOf('0104020506')==-1){
							$('#unsafeact-remove').css('display','none');
						}else{
							$('#unsafeact-remove').css('display','');
						}
						if(resources.indexOf('0104020507')==-1){
					...
					if(resources.indexOf('0104020505')==-1){
						...
						if(loginId==editor || $.inArray(loginId, checkers)!=-1 || roles.indexOf('jtxtgly')!=-1){
							parent.$('#child-win').window({
						 		...
						 		title:'添加不符合项-不安全行为',
						 		cache:false,
						 		content:'<iframe src="${pageContext.request.contextPath}/operatingdepartment/check/periodical_check_inconformity_add_unsafeAct" frameborder="0" width="100%" height="100%"/>'
						 	});
						}else{
