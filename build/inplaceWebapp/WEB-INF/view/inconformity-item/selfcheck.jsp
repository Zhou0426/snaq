...
<script type="text/javascript">
	...
	$(function(){
		...
		$('#dg').datagrid({
			...
			queryParams:{checkType:'动态检查',checkerFrom:'区队'},//请求远程数据发送额外的参数
			...
			onSelect:function(index,row){
				if(resources.indexOf('010201')==-1){
					$('#unsafecondition-add').css('display','none');
				}
				if(resources.indexOf('010202')==-1){
					$('#unsafecondition-delete').css('display','none');
				}else{
					if(loginId==row.editor.personId || (row.checkers.num>0 && $.inArray('${sessionScope.pId}', row.checkers.personIds.split(','))!=-1) || roles.indexOf('dwxtgly')!=-1){
						$('#unsafecondition-delete').css('display','');
					}else{
						$('#unsafecondition-delete').css('display','none');
					}
				}
				if(resources.indexOf('010203')==-1){
					...
					if(resources.indexOf('010201')==-1){
						$('#unsafecondition-add').css('display','none');
						 ...
						 $('#win').window({
				 				...
				 				title:'添加不符合项-隐患',
				 				cache:false,
				 				content:'<iframe src="${pageContext.request.contextPath}/inconformity/item/selfcheck_add" frameborder="0" width="100%" height="100%"/>'
				 		});
					}
				}
			},{
				...
				handler:function(){
					if(resources.indexOf('010202')==-1){
						...
						if(rows.length==0){
							...
							//判断是否为录入人或dwxtgly或检查人
							if(loginId==rows[0].editor.personId || (rows[0].checkers.num>0 && $.inArray('${sessionScope.pId}', rows[0].checkers.personIds.split(','))!=-1) || roles.indexOf('dwxtgly')!=-1){
						...
						if(rows.length==1){
							if(loginId==rows[0].editor.personId || (rows[0].checkers.num>0 && $.inArray('${sessionScope.pId}', rows[0].checkers.personIds.split(','))!=-1) || roles.indexOf('dwxtgly')!=-1){
								$('#win').window({
									...
									title:"修改记录",
									cache:false,
									content:'<iframe src="${pageContext.request.contextPath}/inconformity/item/selfcheck_update" frameborder="0" width="100%" height="100%"/>'
								});
							}else{
		...
		$('#dg2').datagrid({
			...
			idField:"id",// id字段为标识字段,那么此字段状态将会被dg记录下来
			//title:"动态检查",
			queryParams:{checkTypeSn:'0',checkerFromSn:'0'},//请求远程数据发送额外的参数
			fitColumns:true,/*列宽自动*/
			fit:true,
			striped:true,/*斑马线效果  */
			nowrap:false,/*数据同一行*/
			...
			onSelect:function(index,row){
				//第二层权限限制
				if(resources.indexOf('010205')==-1){
					$('#unsafeact-add').css('display','none');
				}
				
				if((row.editor!="无"&&loginId==row.editorId) || (row.checkers!="无"&& $.inArray('${sessionScope.pId}', row.checkersId.split(','))!=-1) || roles.indexOf('dwxtgly')!=-1){
						if(resources.indexOf('010206')==-1){
							$('#unsafeact-remove').css('display','none');
						}else{
							$('#unsafeact-remove').css('display','');
						}
						if(resources.indexOf('010207')==-1){
					...
					if(resources.indexOf('010205')==-1){
						$('#unsafeact-add').css('display','none');
					 	...
					 	$('#win2').window({
					 		...
					 		title:'不符合项-不安全行为',
					 		cache:false,
					 		content:'<iframe src="${pageContext.request.contextPath}/inconformity/item/selfcheck_add_unsafeAct" frameborder="0" width="100%" height="100%"/>'
					 	});
					}
				}
			},{
				...
				handler:function(){
					if(resources.indexOf('010206')==-1){
						...
						if(rows.length==0){
							...
							if((rows[0].editor!="无"&&loginId==rows[0].editorId) || (rows[0].checkers!="无"&& $.inArray('${sessionScope.pId}', rows[0].checkersId.split(','))!=-1) || roles.indexOf('dwxtgly')!=-1){
								$.messager.confirm('删除确认','您确定要删除选中的数据吗？',function(r){
									if(r){
										var ids="";
										for(var i=0;i<rows.length;i++){
						...
						if(rows.length==1){
							if((rows[0].editor!="无"&&loginId==rows[0].editorId) || (rows[0].checkers!="无"&& $.inArray('${sessionScope.pId}', rows[0].checkersId.split(','))!=-1) || roles.indexOf('dwxtgly')!=-1){
								$('#win2').window({
									...
									title:"修改记录",
									cache:false,
									content:'<iframe src="${pageContext.request.contextPath}/inconformity/item/selfcheck_update_unsafeact" frameborder="0" width="100%" height="100%"/>'
