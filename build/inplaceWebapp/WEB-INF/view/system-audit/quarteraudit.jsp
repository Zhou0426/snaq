...
<script type="text/javascript">
	...
	$(function(){
		...
		$('#dg').datagrid({
			...
			idField:'id',
			//title:'季度审核',
			queryParams:{'year':new Date().getUTCFullYear(),type:1},
			fit:true,
			stript:true,
			nowrap:false,
			...
			onBeforeCellEdit:function(index,field){
				...
				if(resources.indexOf("080206")!=-1 && (roles.indexOf("jtxtgly")!=-1 || rows[0].editor.personId=='${personId}')){
					return true;
				}else{
			...
			onCheck:function(index, row){
				//添加
				if(resources.indexOf("080201")==-1){
					$('#add-button').css('display','none');
				}else{
					$('#add-button').css('display','');
				}
				//删除
				if(resources.indexOf("080202")==-1){
					$('#delete').css('display','none');
				}else{
					if(roles.indexOf("jtxtgly")!=-1 || row.editor.personId=='${personId}'){
						$('#delete').css('display','');
					}else{
						$('#delete').css('display','none');
					}
				...
				//进入审核打分
				if(resources.indexOf("080204")==-1){
					$('#score').css('display','none');
				}else{
					if(roles.indexOf("jtxtgly")!=-1 || row.editor.personId=='${personId}' || row.auditTeamLeader.personId=='${personId}' || $.inArray("${personId}", row.auditors.personIds.split(','))!=-1){
						$('#score').css('display','');
					}else{
						$('#score').css('display','none');
					}
			...
			toolbar:[{
				...
				handler:function(){
					if(resources.indexOf("080201")==-1){
						...
						var year=$('#cc1').combobox('getValue');
						var month=$('#cc2').combobox('getValue');
						var value2=$('#cc3').combobox('getValue');
						var value=$('#cc4').combobox('getValue');
						if(value!="" && value2!="" && year!="" && month!=""){
							//添加
							$('#add').dialog({    
						    title: '添加审核信息',    
						    width: 350,    
						    ...
						    modal: true,
						    resizable:true,
						    content:'<iframe src="${pageContext.request.contextPath}/system/audit/quarteraudit_add" frameborder="0" width="100%" height="100%"/>'  
							});  
						}else{
							$.messager.show({
								title:'提示消息',
								msg:'为保证插入信息的完整性，请在上方下拉框中选择年份、季度、单位类型和评分标准！',
								timeout:1000,
								showType:'show',
								style:{
									right:'',
				...
				handler:function(){
					if(resources.indexOf("080202")==-1){
						...
						if(rows.length==0){
							...
							//是否为集团管理员或录入人
							if(roles.indexOf("jtxtgly")!=-1 || rows[0].editor.personId=='${personId}'){
						...
						if(rows.length==1){
							if(roles.indexOf('jtxtgly')!=-1 || rows[0].editor.personId=='${personId}'){
								$('#add').dialog({    
								    ...
								    modal: true,
								    resizable:true,
								    content:'<iframe src="${pageContext.request.contextPath}/system/audit/quarteraudit_update" frameborder="0" width="100%" height="100%"/>'  
									});
							}else{
				...
				handler:function(){
					if(resources.indexOf("080204")==-1){
						...
						var rows=$("#dg").datagrid('getChecked');
						if(rows.length==1){
							if(roles.indexOf("jtxtgly")!=-1 || rows[0].editor.personId=='${personId}' || rows[0].auditTeamLeader.personId=='${personId}' || $.inArray("${personId}", rows[0].auditors.personIds.split(','))!=-1){
								$('#add').dialog({    
								    title: '审核打分',    
								    width:900,    
								    height:400,     
					...
					if(resources.indexOf("080205")==-1){
						...
						if(rows.length==1){
							if(rows[0].finalScore!=null){
								...
								input3.attr("name","departmentName");
								input3.attr("value",rows[0].auditedDepartment.departmentName);
								//将表单放入body
								$("body").append(form);
								form.append(input1);
								form.append(input2);
				...
				handler:function(){
					...
					input3.attr("value",$('#cc2').combobox('getValue'));
			...
			columns:[[
				...
				{field:'remark',title:'备注(双击可编辑)',width:'10%',editor:'textbox'},
				{field:'systemAuditType',title:'检查来自',width:'10%'},
...
<body style="margin: 1px;">
	<input type="hidden" id="type" value="季度审核"/>
	<table id="dg"></table>
	<div id="add"></div>
	<div id="win"></div>
